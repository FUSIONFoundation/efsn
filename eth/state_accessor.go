...
func (eth *Ethereum) stateAtBlock(block *types.Block, reexec uint64, base *state.StateDB, checkLive bool) (statedb *state.StateDB, err error) {
	...
	if checkLive {
		statedb, err = eth.blockchain.StateAt(block.Root(), block.MixDigest())
		if err == nil {
			return statedb, nil
		}
	}
	if base != nil {
		...
		if !checkLive {
			statedb, err = state.New(current.Root(), current.MixDigest(), database)
			if err == nil {
				return statedb, nil
			}
		}
	...
	for current.NumberU64() < origin {
		...
		statedb, err = state.New(root, common.Hash{}, database)
		if err != nil {
