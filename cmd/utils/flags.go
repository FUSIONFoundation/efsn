...
import (
	...
	"github.com/FusionFoundation/efsn/v5/common/fdlimit"
	...
	"github.com/FusionFoundation/efsn/v5/metrics/exp"
	"github.com/FusionFoundation/efsn/v5/metrics/influxdb"
...
var (
	...
	TestnetFlag = &cli.BoolFlag{
		Name:     "testnet",
		Usage:    "Testnet network: pre-configured Fusion test network",
		Category: flags.EthCategory,
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	if ctx.IsSet(ResyncFromHeightFlag.Name) {
		core.ResyncFromHeight = ctx.Uint64(ResyncFromHeightFlag.Name)
	}
	...
	case ctx.Bool(DevnetFlag.Name):
		...
		if ctx.IsSet(DevnetAddrFlag.Name) {
			devnetAddr := common.HexToAddress(ctx.String(DevnetAddrFlag.Name))
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb ethdb.Database) {
	...
	if config.DaTong != nil {
		engine = datong.New(config.DaTong, chainDb)
	} else {
		Fatalf("No Consensus Engine!")
	}
	if gcmode := ctx.String(GCModeFlag.Name); gcmode != "full" && gcmode != "archive" {
		Fatalf("--%s must be either 'full' or 'archive'", GCModeFlag.Name)
	}
