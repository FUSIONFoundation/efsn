...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	if ok {
		...
		if _, ok := s.engine.(*datong.DaTong); ok {
			header := fullBackend.CurrentHeader()
			if statedb, _, err := fullBackend.StateAndHeaderByNumber(context.Background(), rpc.BlockNumber(header.Number.Int64())); err == nil {
				if tickets, err := statedb.AllTickets(); err == nil {
					ticketNumber = new(big.Int).SetUint64(tickets.NumberOfTickets())
					etherbase, _ := fullBackend.Coinbase()
