...
// GetAssetId returns a Fusion Asset ID or Swap ID
func (tx *Transaction) GetAssetId() common.Hash {
	return rlpHash(&LegacyTx{
		Nonce:    tx.Nonce(),
		To:       tx.To(),
		Value:    tx.Value(),
		Gas:      tx.Gas(),
...
func (s TxByPriceAndTime) Less(i, j int) bool {
	order1 := s[i].tx.GetOrder()
	order2 := s[j].tx.GetOrder()
